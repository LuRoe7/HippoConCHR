...
for (bvar in behavvars) {
  for (nvar in neurovars) {
    ...
    df = df_napls_mlr_imp[c(paste0(bvar,"_CH"),paste0(nvar,"_CH"),"CHR",paste0(bvar,"_SE"),paste0(nvar,"_SE"),
                            paste0("nt_",nvar),paste0("nt_",bvar),
    ...
    # keep only subjects with at least 3 time points
    df = df[df[[paste0("nt_", nvar)]] > 2 & df[[paste0("nt_", bvar)]] > 2, ]
    ...
    # drop standard error and number of timepoint columns again
    df = df[ , !grepl("_SE$", names(df)) ]
    df = df[ , !grepl("nt_", names(df)) ]
