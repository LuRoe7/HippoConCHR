...
for (bvar in behavvars) {
  for (nvar in neurovars) {
    ...
    # build a data frame keeping only the columns needed for this model
    df = df_napls_mlr_imp[c(paste0(bvar,"_CH"),paste0(nvar,"_CH"),"CHR",paste0(bvar,"_SE"),paste0(nvar,"_SE"),
                            paste0(bvar,"_BL"),paste0(nvar,"_BL"),
                            "age","female","site02","site03","site04","site05","site06","site07","site08","site09")]
    
    # remove subjects who have missing values on either change and baseline score, since Mplus needs those present
    df = df[complete.cases(df[c(paste0(bvar, "_CH"), paste0(nvar, "_CH"),paste0(bvar, "_BL"), paste0(nvar, "_BL"))]), ]
    
    # get average measurement error variance for bvar by squaring its mean standard error across subjects
    se_bvar = mean(df[[paste0(bvar,"_SE")]],na.rm=T)^2
    # get average measurement error variance for nvar the same way
    se_nvar = mean(df[[paste0(nvar,"_SE")]],na.rm=T)^2
    # drop the standard error columns now that they've been used to compute error variances
    df = df[ , !grepl("_SE$", names(df)) ]
    # standardize specific columns by mean (e.g. baseline values
    scols = c(paste0(bvar, "_BL"), paste0(nvar, "_BL")) 
    df[scols] = scale(df[scols],scale=F)
    # build the path for the data file that Mplus will read
    datafile = file.path(wd, paste0("NaplsData_", nvar, "_", bvar, ".dat"))
    # build the path for the Mplus input (.inp) file
    inpfile = file.path(wd, paste0("NaplsData_", nvar, "_", bvar, ".inp"))
    # write the data to a tab-delimited file with no header/row names, as Mplus expects
    write.table(df,datafile,quote = F,row.names = F, col.names = F,append = F, sep = "\t")
    
    # build the full Mplus syntax as one text string, inserting variable names dynamically
    inp_text = paste0("
    ...
    LV_", bvar, " on LV_", nvar_mplus, " (b1)
               CHR
               ", nvar_mplus, "xCHR (b2) 
               ", bvar, "_BL ", nvar_mplus, "_BL 
               age female site02 site03 site04
               site05 site06 site07 site08
               site09;
    
    LV_", nvar_mplus, " WITH CHR ", bvar, "_BL ", nvar_mplus, "_BL 
                        age female site02 site03 site04
                        site05 site06 site07 site08
                        site09;
    
    CHR WITH ", bvar, "_BL ", nvar_mplus, "_BL
        age female site02 site03 site04
        site05 site06 site07 site08
        site09;
    
    ", bvar, "_BL WITH ", nvar_mplus, "_BL
        age female site02 site03 site04
        site05 site06 site07 site08
        site09;
    
    ", nvar_mplus, "_BL WITH age female site02 site03 site04
    site05 site06 site07 site08
    site09;
...
behavvars_mplus = c("LV_NEGS.ON","LV_DEPS.ON","LV_GAF.ON")
# create empty list
...
LV_", bvar, " on LV_", nvar_mplus, "
           ", bvar, "_BL ", nvar_mplus, "_BL 
           age female site02 site03 site04
           site05 site06 site07 site08
           site09;

LV_", nvar_mplus, " WITH ", bvar, "_BL ", nvar_mplus, "_BL 
                    age female site02 site03 site04
                    site05 site06 site07 site08
                    site09;
...
inp_text = paste0("
Title:      LATENT VARIABLE REGRESSION (MULTIGROUP)
...
! define grouping variable (0 = HC, 1 = CHR)
classes = g(2);
knownclass = g (CHR=0 CHR=1);
...
! measurement model (same across groups unless specified otherwise)
%OVERALL%
...
LV_", bvar, " on LV_", nvar_mplus, " (b_ctrl)   ! slope in controls
           ", bvar, "_BL ", nvar_mplus, "_BL
           age female site02 site03 site04
           site05 site06 site07 site08 site09;

! -------------------------
! GROUP 2: CHR (patients)
! -------------------------
%g#2%

LV_", bvar, " on LV_", nvar_mplus, " (b_pat)    ! slope in CHR
           ", bvar, "_BL ", nvar_mplus, "_BL
           age female site02 site03 site04
           site05 site06 site07 site08 site09;

! -------------------------
! (Co)variances (applied overall)
! -------------------------
%OVERALL%
...
Model Constraint:
    NEW(diff);
    diff = b_pat - b_ctrl;   ! difference in slopes (CHR - HC)
...
TITLE:
    LATENT VARIABLE REGRESSION - MULTIGROUP MODEL;
...
VARIABLE:
    ...
    ! Define CHR as the grouping variable.
    ! CHR = 0 -> HC
    ! CHR = 1 -> CHR
    GROUPING IS CHR (0 = HC 1 = CHR);


ANALYSIS:

    ! Standard multigroup model; no latent interaction is required
    ESTIMATOR = MLR;
    STARTS=20;
    STITERATION=50000;
...
MODEL:
    ...
    ! Define latent hippocampal change variable from observed change score
    LV_", nvar_mplus, " BY ", nvar_mplus, "_CH;

    ! Fix the residual variance to the estimated average measurement error
    ! variance, thereby correcting the latent variable for measurement error
    ", nvar_mplus, "_CH @ ", format(se_nvar, scientific = FALSE), ";


    ! Define latent negative-symptom change variable
    LV_", bvar, " BY ", bvar, "_CH;
    ...
    ! Allow LV_HI to covary with all baseline covariates
    ! and demographic/site variables
    LV_", nvar_mplus, " WITH
        ", bvar, "_BL
        ", nvar_mplus, "_BL
        age
    ...
    ! Allow baseline behavioral and hippocampal variables to covary
    ", bvar, "_BL WITH
        ", nvar_mplus, "_BL
        age
        female
    ...
    ! Allow baseline hippocampal variable to covary with demographic
    ! and site variables
    ", nvar_mplus, "_BL WITH
        age
        female
    ...
    ! Allow age to covary with sex and site variables
    age WITH
        female
        site02
        site03
    ...
    ! Allow sex to covary with site variables
    female WITH
        site02
        site03
        site04
    ...
    ! Allow site variables to covary
    site02 WITH
        site03
        site04
        site05
...
MODEL HC:
    ...
    ! -------------------------------------------------------------
    ! Effect of latent hippocampal change on latent negative-
    ! symptom change in healthy controls
    !
    ! b_HC = LV_HI -> LV_NEGS effect in HC
    ! -------------------------------------------------------------
...
MODEL CHR:
    ...
    ! -------------------------------------------------------------
    ! Effect of latent hippocampal change on latent negative-
    ! symptom change in CHR
    !
    ! b_CHR = LV_HI -> LV_NEGS effect in CHR
    ! -------------------------------------------------------------

    LV_", bvar, " ON
        LV_", nvar_mplus, " (b_CHR)
        ", bvar, "_BL
        ", nvar_mplus, "_BL
        age
        female
...
! =========================================================================
! TEST WHETHER THE LV_HI EFFECT DIFFERS BETWEEN HC AND CHR
! =========================================================================

MODEL CONSTRAINT:

    ! Calculate the difference between the two group-specific effects
    !
    ! diff > 0  -> effect is larger in CHR
    ! diff < 0  -> effect is larger in HC
    !
    ! The estimate, SE, two-tailed p-value and confidence interval
    ! for this NEW parameter are reported by Mplus.
...
SAVEDATA:
    ESTIMATES = power_", nvar, "_", bvar, ".dat;
